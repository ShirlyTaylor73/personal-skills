...
def validate_completed_job_source(
    ...
    *,
    ...
    if job.get("source_provenance") == "deterministic-mirror":
        raise SystemExit(
            f"job {job_id} uses deterministic-mirror provenance; "
            "running-left must be generated independently for realistic humans"
        )
...
def main() -> None:
    ...
    qa_dir = run_dir / "qa"
    raw_frames_dir = run_dir / "frames_raw"
    matted_frames_dir = run_dir / "frames"
    ...
    run(
        [
            sys.executable,
            str(scripts_dir / "matte_frames.py"),
            "--frames-dir",
            str(raw_frames_dir),
            "--output-dir",
            str(matted_frames_dir),
            "--qa-dir",
            str(qa_dir),
            "--target-height-ratio",
            "0.94",
